...
import { getCaseHistory } from "../../services/caseApi";

/**
 * Query keys for case assignment history
 */
export const caseAssignmentKeys = {
  all: ["caseAssignmentHistory"],
  byCase: (caseId) => [...caseAssignmentKeys.all, "case", caseId],
  byCaseFiltered: (caseId, filters) => [
    ...caseAssignmentKeys.byCase(caseId),
    "filtered",
    filters,
  ],
};
...
/**
 * Hook to fetch assignment history for a specific case
 * Filters the history to show only assignment-related changes
 */
export function useCaseAssignmentHistory(caseId, options = {}) {
  const { filters = {}, enabled = true } = options;

  return useQuery({
    queryKey: caseAssignmentKeys.byCaseFiltered(caseId, filters),
    queryFn: async () => {
      if (!caseId) {
        throw new Error("Case ID is required");
      }

      // Fetch all history for the case
...
/**
 * Hook to get assignment statistics for a case
 */
export function useCaseAssignmentStats(caseId) {
  return useQuery({
    queryKey: [...caseAssignmentKeys.byCase(caseId), "stats"],
    queryFn: async () => {
      if (!caseId) {
        throw new Error("Case ID is required");
      }

...
/**
 * Custom hook for cases created by the current user
 * Simplified version of useCaseTable with user-specific filtering
 */
export function useCreatedByMeTable() {
  const { user } = useAuth();
  ...
  // Filter state (simplified for created cases)
  ...
  const [pageSize, setPageSize] = useUserPreference(
    "createdCaseTablePageSize",
    25
  );

  // Convert filters for backend API call + add user creation filter
  const backendFilters = useMemo(() => {
    const converted = {
      createdBy: currentUserId, // Filter for current user's created cases
      isDeleted: false,
    };
